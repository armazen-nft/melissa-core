"""
VectorStore — local FAISS index (IVF+PQ) + SQLite metadata.

- FAISS IVF256,PQ32: scales to millions of chunks, CPU fast.
  Auto-falls back to IndexFlatL2 when corpus < 1 000 chunks.
- SQLite (sqlitedict) for chunk text + metadata keyed by chunk id.
- File dedup via SHA256 hash registry.
"""
...
class VectorStore:
    ...
    def add(self, chunks: list[EmbeddedChunk]) -> int:
        new_chunks = [c for c in chunks if not self._chunk_exists(c.id)]
        if not new_chunks:
            return 0

        vecs = np.stack([c.embedding for c in new_chunks]).astype(np.float32)
        total_after = len(self._id_map) + len(new_chunks)
        self._ensure_index(total_after, vecs)

        if hasattr(self._index, "is_trained") and not self._index.is_trained:
            log.info("Training FAISS IVF index on %d vectors…", len(vecs))
        ...
        for chunk in new_chunks:
            ...
            db[chunk.id] = json.dumps(doc, default=str)
        db.commit()
        self._save()

        log.info("Added %d chunks (total=%d)", len(new_chunks), len(self._id_map))
    ...
    def _ensure_index(self, total_after: int, vecs: np.ndarray) -> None:
        ...
        if self._index is None:
            if total_after < FLAT_THRESHOLD or self.index_type == "Flat":
                log.info("Creating IndexFlatL2 (dim=%d)", self.dim)
                self._index = faiss.IndexFlatL2(self.dim)
            else:
                itype = self.index_type or "IVF256,PQ32"
                log.info("Creating FAISS index %s (dim=%d)", itype, self.dim)
                self._index = faiss.index_factory(self.dim, itype, faiss.METRIC_L2)
        elif (
            isinstance(self._index, faiss.IndexFlatL2)
            and total_after >= FLAT_THRESHOLD
            and self.index_type != "Flat"
        ):
            log.info("Migrating Flat → IVF (corpus=%d)", total_after)
            itype = self.index_type or "IVF256,PQ32"
            new_index = faiss.index_factory(self.dim, itype, faiss.METRIC_L2)
            if self._id_map:
                old_vecs = self._index.reconstruct_n(0, len(self._id_map))
                new_index.train(old_vecs)
    ...
    def _load(self) -> None:
        ...
        if self._hash_path.exists():
            with open(self._hash_path) as f:
                self._file_hashes = set(json.load(f))

    def _save_hashes(self) -> None:
        with open(self._hash_path, "w") as f:
            json.dump(list(self._file_hashes), f)
